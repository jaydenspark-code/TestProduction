...
-- Run this in Supabase Studio SQL Editor if needed

-- Step 1: Check if we're connected
SELECT 'Supabase connection test' as status, NOW() as timestamp;

-- Step 2: Create the agent applications schema
...
-- Add missing columns to existing agent_applications table if they don't exist
DO $$ 
BEGIN
  -- Add application_data column if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'agent_applications' AND column_name = 'application_data') THEN
    ALTER TABLE agent_applications ADD COLUMN application_data JSONB DEFAULT '{}';
  END IF;
  
  -- Add social_media_profiles column if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'agent_applications' AND column_name = 'social_media_profiles') THEN
    ALTER TABLE agent_applications ADD COLUMN social_media_profiles JSONB DEFAULT '{}';
  END IF;
  
  -- Add AI review columns
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'agent_applications' AND column_name = 'ai_score') THEN
    ALTER TABLE agent_applications ADD COLUMN ai_score DECIMAL(5,2) DEFAULT 0;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'agent_applications' AND column_name = 'ai_decision') THEN
    ALTER TABLE agent_applications ADD COLUMN ai_decision VARCHAR(20) DEFAULT 'pending';
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'agent_applications' AND column_name = 'ai_decision_reason') THEN
    ALTER TABLE agent_applications ADD COLUMN ai_decision_reason TEXT;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'agent_applications' AND column_name = 'ai_compliance_details') THEN
    ALTER TABLE agent_applications ADD COLUMN ai_compliance_details JSONB;
  END IF;
  
  -- Add admin review columns
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'agent_applications' AND column_name = 'admin_status') THEN
    ALTER TABLE agent_applications ADD COLUMN admin_status VARCHAR(20) DEFAULT 'pending';
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'agent_applications' AND column_name = 'admin_reviewed_by') THEN
    ALTER TABLE agent_applications ADD COLUMN admin_reviewed_by UUID REFERENCES auth.users(id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'agent_applications' AND column_name = 'admin_review_reason') THEN
    ALTER TABLE agent_applications ADD COLUMN admin_review_reason TEXT;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'agent_applications' AND column_name = 'admin_review_date') THEN
    ALTER TABLE agent_applications ADD COLUMN admin_review_date TIMESTAMP WITH TIME ZONE;
  END IF;
  
  -- Add final_status column (this is the one causing the error)
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'agent_applications' AND column_name = 'final_status') THEN
    ALTER TABLE agent_applications ADD COLUMN final_status VARCHAR(20) DEFAULT 'pending';
  END IF;
  
  -- Add other missing columns
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'agent_applications' AND column_name = 'is_reapplication') THEN
    ALTER TABLE agent_applications ADD COLUMN is_reapplication BOOLEAN DEFAULT false;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'agent_applications' AND column_name = 'previous_application_id') THEN
    ALTER TABLE agent_applications ADD COLUMN previous_application_id UUID;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'agent_applications' AND column_name = 'processed_at') THEN
    ALTER TABLE agent_applications ADD COLUMN processed_at TIMESTAMP WITH TIME ZONE;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'agent_applications' AND column_name = 'created_at') THEN
    ALTER TABLE agent_applications ADD COLUMN created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'agent_applications' AND column_name = 'updated_at') THEN
    ALTER TABLE agent_applications ADD COLUMN updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
  END IF;
END $$;
...
BEGIN
  ...
  -- If approved, activate user as agent (only if agent_profiles table exists)
  IF p_decision = 'approved' THEN
    -- Check if agent_profiles table exists first
...
AND (table_name LIKE '%application%' OR table_name = 'social_media_platforms');

-- Step 4: Check platform requirements were inserted
SELECT platform_name, min_followers 
FROM social_media_platforms 
ORDER BY min_followers DESC;

-- Step 5: Test the AI review function
SELECT 'process_ai_application_review'::text as function_name,
       'Available'::text as status
WHERE EXISTS (
    SELECT 1 FROM information_schema.routines 
    WHERE routine_name = 'process_ai_application_review'
);

-- Setup complete message
SELECT 'Agent Applications System Setup Complete!' as message;
