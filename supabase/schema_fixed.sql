...
-- CRITICAL FIX: Add INSERT policy for user registration
CREATE POLICY "Allow user registration" ON users
    FOR INSERT WITH CHECK (auth.uid() = id);

-- Allow public read access for referral codes (needed for referral validation)
CREATE POLICY "Public read access for referral validation" ON users
...
-- Campaigns policies  
CREATE POLICY "Public read access for campaigns" ON campaigns
    FOR SELECT USING (true);
