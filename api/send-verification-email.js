// Vercel API endpoint to send verification emails via SendGrid
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('📧 API: Received request:', { email, fullName, hasToken: !!verificationToken });
    ...
    const appUrl = process.env.VITE_APP_URL || process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'http://localhost:5173';
    ...
    console.log('📤 API: Making request to SendGrid...');
    ...
    console.log('📨 API: SendGrid response status:', response.status);
